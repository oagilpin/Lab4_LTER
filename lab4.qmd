...
#| echo: false
#| out.width: "100%"
knitr::include_graphics('img/carapace_width_by_site.png')
knitr::include_graphics('img/water_temp_sd_vs_carapace_width.png')
```
...
#How does this relate to the original hypothesis based on the graph we made at the beginning of this section? Our original hypothesis suggested that forest harvesting affects Cutthroat trout biomass, but the initial boxplot showed no clear visual difference between clear-cut and old-growth forests. Statistical tests revealed unequal variances and skewed data, requiring a log transformation. The t-test on transformed data determined whether the weight difference was significant, confirming or challenging our initial visual interpretation.
...
#P1**1.** Conduct a chi-square test similar to the one carried out above, but test for a relationship between forest type (`section`) and channel unit (`unittype`) for *Coastal giant salamander* abundance. *Keep all unittypes* instead of filtering any like we did for the Cutthroat trout (10 pts.) --	

#P1-1 Answer: The chi-square test for Coastal giant salamander abundance showed a significant relationship between forest type and channel unit (p-value < 2.2e-16).
...
#P1**2.** Test the hypothesis that there is a significant difference in species biomass between clear cut and old growth forest types for the *Coastal Giant salamander*. (10 pts.)

#P2-2 Answer: The two-sample t-test for biomass revealed a significant difference in the species biomass between clear-cut and old-growth forest types for Coastal Giant Salamanders (p-value = 6.056e-06).

<br>
```{r}
sally_data <- and_vertebrates %>%
  filter(species == "Coastal giant salamander", section %in% c("Clear-cut", "Old-growth")) %>%
...
#P1**3.** Test the correlation between body length (snout to fork length) and body mass for *Cutthroat trout*. (Hint: run `?and_vertebrates` to find which length variable represents snout to fork length) (10 pts.)

#P1-3 Answer: The Pearson correlation analysis indicated a strong positive correlation between snout-to-fork length and weight for Cutthroat trout (r = 0.912, p < 2.2e-16). This tell us that as the snout-to-fork length increases, the weight of the trout also increases in a linear relationship. This result is highly statistically significant.
...
correlation_result <- cor.test(trout_data$length_1_mm, trout_data$weight_g)
correlation_result
```
```{r}
cat("Pearson correlation result:", "\n", 
    "correlation coefficient = ", correlation_result$estimate, 
    ", p-value = ", correlation_result$p.value, "\n")
```
```{r}
trout_data_sf <- and_vertebrates %>%
  filter(species == "Cutthroat trout") %>%
  select(length_1_mm, weight_g) %>%
  drop_na()

correlation_result_sf <- cor.test(trout_data_sf$length_1_mm, trout_data_sf$weight_g)

cat("Pearson correlation result:", "\n", 
    "correlation coefficient = ", correlation_result_sf$estimate, 
    ", p-value = ", correlation_result_sf$p.value, "\n")
```
...
#P2-1 Answer: ANOVA was conducted to analyze differences in carapace width among three sites: GTM, DB, and PIE. The results of the ANOVA revealed significant differences in carapace width between the sites (F(2, 83) = 60.02, p < 2e-16). The post-hoc Tukey’s HSD test results showed significant differences between all pairs of sites: GTM and DB (p < 0.00001), PIE and DB (p < 0.00001), and PIE and GTM (p < 0.00001). These findings suggest that carapace width varies significantly across the sites, which could be attributed to environmental or geographical factors, such as latitude.
```{r}
library(ggplot2)
...
pie_crab %>%
  ...
  geom_boxplot() +
  labs(x = "Site", y = "Carapace Width (size)", 
...
```
```{r}
ggsave("carapace_width_by_site.png", width = 8, height = 6)
```
...
#P2-2 Answer: The regression model showed that the effect of annual water temperature variation on carapace width was not statistically significant (t(390) = 0.632, p = 0.528). The coefficient for water_temp_sd was 0.09938, indicating that a one-unit increase in water temperature variation is associated with an increase of 0.09938 units in carapace width, not significant. The model’s R-squared value was very low (R² = 0.001), which sugeests water temperature variation only explains a 0.1% of the variation in carapace width. This means the analysis provided no evidence to support a meaningful relationship between water temperature variation and carapace width in pie crabs.
...
ggplot(pie_crab, aes(x = water_temp_sd, y = size)) +
  geom_point(alpha = 0.5) +  # Scatter plot for the data points
  geom_smooth(method = "lm", color = "purple", se = FALSE) +  # Line of best fit
  labs(title = "Effect of Water Temperature SD on Carapace Width",
       x = "Water Temperature SD (Annual Variation)",
       y = "Carapace Width (mm)") +
  theme_minimal()
```
```{r}
ggsave("water_temp_sd_vs_carapace_width.png", width = 8, height = 6)
```
...
#P2-3 Answer: The multiple linear regression analysis revealed that latitude significantly predicted carapace width (p < 2e-16), while air temperature variation (p = 0.1730) and water temperature variation (p = 0.3254) were not significant predictors.
